...
function OfferPage() {
  ...
  const { offer, suggestions, reviews, fetchingOfferStatus } = useAppSelector((state) => state);
  ...
  return (
    <div className="page">
      ...
      {fetchingOfferStatus === RequestStatus.Error && <h1>Error</h1>}
      {fetchingOfferStatus === RequestStatus.Pending && <Spinner />}
    </div>
  );
}
